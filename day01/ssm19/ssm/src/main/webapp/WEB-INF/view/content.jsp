        ...
        <div class="row">
			  ...
			  <div class="col-lg-2">
				<div class="input-group">
					...
					<select class="form-control" name="yid" id = "txt_search_roleid">
						<option value="0">---请选择---</option>
						<c:forEach items="${ylist }" var="y">
						 	<option value="${y.yid }">${y.yname }</option>
						</c:forEach>
                	</select>
				</div>
			 </div>
			...
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveContent.htm">
						<input type="hidden" name="cid" id="hidden_txt_userid" value=""/>
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>广告标题：</td>
								<td><input type="text" id="contenttitle" name="contenttitle" class="form-control" aria-required="true" required/></td>
								
								<td>广告链接：</td>
								<td><input type="text" id="contenturl" name="contenturl" class="form-control" aria-required="true" required/></td>
							</tr>
							<tr>
								<td>图片路径：</td>
								<td><input type="text" id="picpath" name="picpath" class="form-control" aria-required="true" required/></td>
							...
							<tr>
								<td>类型：</td>
								<td colspan="4">
									<select class="form-control" name="yid" id = "yid" aria-required="true" required>
										<option value="">---请选择---</option>
										<c:forEach items="${ylist }" var="y">
										 	<option value="${y.yid }">${y.yname }</option>
										</c:forEach>
...
<div class="modal fade" id="modal_user_export" role="dialog" aria-labelledby="modal_user_export" aria-hidden="true">
	...
	function initEditUser(getSelection){
		$('#hidden_txt_userid').val(getSelection.cid);
	...
	$("#export_user_btn").click(function(){
		//往后端发送请求
		location.href="exportContent.htm";	
	$('#modal_user_del').modal('hide');   		
	});
	</script>
